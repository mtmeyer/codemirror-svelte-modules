<script lang="ts">
  ...
  /**
   * This will reset the CodeMirror view to the provided state, which will trigger a initialisation of the extensions provided.
   * This should only be used when you need to completely reset the state, rather than update the existing state.
   *
   * @param value - The content string you want to update the state with
   */
  export const forceUpdateValue = (value: string) => {
    view.setState(
      EditorState.create({ doc: value, extensions: [...initialExtensions(), ...extensions] })
    );
  };
